...
public:
    ...
    void hysthereses(vector<vector<Pixel>>& img, int low, int high, int xstart, int ystart) {
        ...
        if (xstart < 0 || xstart >= height || ystart < 0 || ystart >= width || img[xstart][ystart].getGray() == 0 || img[xstart][ystart].getGray() == 3) {
            return;
        }
    ...
    vector<vector<Pixel>> dthreshold(vector<vector<Pixel>>& image, int low, int high, string s) {
        ...
        outfile << width << " " << height << endl;
        outfile << maxval << endl;
        vector<vector<Pixel>> start(height, vector<Pixel>(width, Pixel(0, 0, 0)));
        for (int i = 0; i < height; i++) {
            for (int j = 0; j < width; j++) {
    ...
    void lineVote(Line l, vector<vector<int>>& votes, Point start) {
        Point p1 = Point(start.getX() - 150, l.getYfromX(start.getX() - 150));
        Point p2 = Point(start.getX() + 150, l.getYfromX(start.getX() + 150));
        int x0 = p1.getX();
        int x1 = p2.getX();
        int y0 = p1.getY();
        int y1 = p2.getY();
