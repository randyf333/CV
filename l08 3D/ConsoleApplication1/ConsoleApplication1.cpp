...
public:
	...
	void render(vector<Mat>& points, Shape3D shape, vector<Mat>& frames, int& count2, ofstream& out2, ofstream& logfile, Mat& finalMat) { //make frame Change for 8.2
	...
	void partt1(int argc, char* argv[]) {
		...
		for (int times = 0; times < 1000; times++) { //change back to 1000
			...
			projectCheck = true;
			//vertCheck = true;
			render(renderPoints, cube, frames, count2, out2, logFile, finalMat);
			//cout << count2;
			points = newPoints;
