...
public:
    ...
    static void drawPoint(doublePoint center, int** arr, int xsize, int ysize, int color) {
        Circle first(3, roundNum(center.getX()), roundNum(center.getY()), center.getX(), center.getY(), 1);
    ...
    //false on left side, true on right side
    static bool findSide(doublePoint a, doublePoint b, doublePoint point) {
    ...
    static std::vector<doublePoint> vreadFromTxt() {
        ...
        if (points.is_open()) {
            while (getline(points, line)) {
                ...
                std::string firsttok = line.substr(0, delim2ind);
                std::string sectok = line.substr(delim2ind + 2, delim2ind);
    ...
    static double compare(doublePoint p1, doublePoint p2, doublePoint p3) {
        return (p3.getY() - p1.getY()) * (p2.getX() - p1.getX()) - (p3.getX() - p1.getX()) * (p2.getY() - p1.getY());
    }

    static void doPart2() { //graham scan
        int** arr = new int* [XSIZE];
        for (int i = 0; i < XSIZE; i++) {
            arr[i] = new int[YSIZE];
            for (int j = 0; j < YSIZE; j++) {
        ...
        vector<doublePoint> points = vreadFromTxt();
        //cout << points.size();
        stack<doublePoint> s;
        ...
        //double angle = compareAngle(p0, points[1]);
        //for (int i = 2; i < points.size(); i++) {
        //    //double newangle = compareAngle(p0, points[i]);
        //    double newangle = compareAngle(points[i], p0);
        //    for (int j = 1; j < i; j++) {
        //        //double oldangle = compareAngle(p0, points[j]);
        //        double oldangle = compareAngle(points[j], p0);
        //        if (newangle > oldangle) {
        //            swap(points[i], points[j]);
        //        }
        //    }
        //}
        sort(points.begin()+1, points.end(), [p0](doublePoint p1, doublePoint p2){return compare(p0,p1,p2) > 0; });//sort angles
        //qsort(&points[1], points.size() - 1, sizeof(doublePoint),compare);
        ...
        for (int i = 1; i < points.size(); i++) {
            doublePoint b = s.top();
            s.pop();
            while (!s.empty() && compare(b,s.top(), points[i])>0) {
                b = s.top();
                s.pop();
            }
            s.push(b);
        ...
        doublePoint fir = s.top();
        s.pop();
        while (!s.empty()) {
            drawPoint(fir, arr, XSIZE, YSIZE, 2);
            //cout << "Draw Point: " << fir.getX() << fir.getY() << endl;
            Point firpoint = toPoint(fir);
            doublePoint sec = s.top();
            Point secpoint = toPoint(sec);
            Line l(firpoint, secpoint); 
            l.drawLine(l.getP1(), l.getP2(), arr, 2);
            fir = sec;
            s.pop();
        }
        doParts::rotateMatrix(XSIZE, YSIZE, arr);
